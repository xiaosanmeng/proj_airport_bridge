...
class Crew:
    ...
    def login(self,crew_zizhi_path,crew_group_path):
        ...
        df_new['location'] = df_new['lounge'] # 工作人员位置，一开始在休息室
        df_new['count'] = 0 # 被派工次数
        df_new['free'] = 0 # 空闲时间：距离上次运行工作时间
        df_new['move_distance'] = 0  # 记录移动距离
        self.dfCrew = df_new
    ...
    def get_near1_people_sim(self,lounge,group, now):
        ''' 
        获取附近 1/4的人员
        '''
        assert lounge in [187,153,60,88]
        ...
        # 找到对应 position 最近人员 并且 空闲状态 
        df_can1 = self.dfCrew[(self.dfCrew['lounge'] == lounge) & (self.dfCrew['status'] == 0) & (self.dfCrew['group'] == group)]  # 该区域的空闲人
        df_can2 =  self.dfCrew[(self.dfCrew['lounge'] == lounge) & (self.dfCrew['status'] == 1) & (self.dfCrew['group'] == group) & (self.dfCrew['end_time'] < now + pd.Timedelta('15 min'))]  # 该区域的即将空闲人
        df_can = pd.concat([df_can1,df_can2])

        return df_can

    def get_near2_people_sim(self,lounge,group, now):
        '''
        获取附近 1/2 空闲人员
        '''
        # 187航站楼 与 153 航站楼连接
    ...
    def get_near3_people_sim(self,lounge,group, now):
        '''
        获取附近 全部区域 空闲人员
        '''
        df_can = self.dfCrew[(self.dfCrew['status'] == 0) & (self.dfCrew['group'] == group)]
        return df_can
    ...
    def _update_people_status(self,name_dict,task:Task,now):
        ...
        for name in name_dict:
            ...
            self.dfCrew.loc[self.dfCrew['name'] == name, 'location'] = task.gate # 工作人员位置
            self.dfCrew.loc[self.dfCrew['name'] == name, 'free'] = 0 # 重置空闲时间
            self.dfCrew.loc[self.dfCrew['name'] == name, 'count'] += 1
    
    def _update_people_status_KM(self, distance, name,task:Task,now):
        '''
        根据分配的任务更新人员状态
        '''
        # 任务指派之后，对应人员的数值得到更新
        self.dfCrew.loc[self.dfCrew['name'] == name, 'status'] = 1 # 将其状态设置为被占用
        self.dfCrew.loc[self.dfCrew['name'] == name, 'work_load'] += task.get_task_duration_base_real() # 增加工作时长
        time = task.taskDuration[0] # int type
        # self.dfCrew.loc[self.dfCrew['name'] == name, 'end_time'] = now + pd.Timedelta(f'{time}minute')# 设置期望工作时间
        self.dfCrew.loc[self.dfCrew['name'] == name, 'end_time'] = task.time + pd.Timedelta(f'{time}minute')# 设置期望工作时间
        self.dfCrew.loc[self.dfCrew['name'] == name, 'gate'] = task.gate # 工作登机口
        self.dfCrew.loc[self.dfCrew['name'] == name, 'location'] = task.gate # 工作人员位置
        self.dfCrew.loc[self.dfCrew['name'] == name, 'free'] = 0 # 重置空闲时间
        self.dfCrew.loc[self.dfCrew['name'] == name, 'count'] += 1
        self.dfCrew.loc[self.dfCrew['name'] == name, 'move_distance'] += distance
    ...
    def record_single_day(self, day):
        ...
        self.dfCrew['gate'] = None # 不需要更新
        self.dfCrew['location'] = self.dfCrew['lounge']  # 位置回到休息室
        self.dfCrew['count'] = 0 # 排班次数清零
        self.dfCrew['free'] = 0  # 空闲时间等待最后更新
        self.dfCrew['move_distance'] = 0 # 移动距离清零
    ...
    def update_status(self,now):
        ...
        # self.dfCrew.loc[(now > self.dfCrew['end_time'] + pd.Timedelta('2 min')) & (self.dfCrew['status'] == 1), 'status'] = 0  # 这里为什么要加2min?
        ...
        # 工作人员工作结束后就返回休息室
        self.dfCrew.loc[(now > self.dfCrew['end_time']) & (self.dfCrew['status'] == 1), 'location'] = self.dfCrew.loc[(now > self.dfCrew['end_time']) & (self.dfCrew['status'] == 1), 'lounge']

        # 工作状态用户改为空闲状态
        self.dfCrew.loc[(now > self.dfCrew['end_time']) & (self.dfCrew['status'] == 1), 'status'] = 0

    def update_status_KM(self, now):
        ''' 
        根据当前的时间恢复状态
        '''
        # 空闲状态用户的空闲时间增加
